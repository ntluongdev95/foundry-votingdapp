...
contract Voting is AutomationCompatibleInterface, Ownable {
    ...
    function vote(VotingMessage calldata vote_msg, bytes calldata signature, address signer) external {
        ...
        checkTime(pool.startAt, pool.endsAt);
        Contestant[] storage contestantsList = s_contestantToPool[vote_msg.poolId];
        ...
        s_contestants[vote_msg.poolId][vote_msg.contestantId].votesCount += 1;
    ...
    function findContestant(uint256 poolId, uint256 contestantId) public view returns (Contestant memory) {
        return s_contestants[poolId][contestantId];
    }
    ...
    {    for (uint i = 1; i <= s_totalPools; i++) {
     ...
     function performUpkeep(bytes calldata performData) external override {
        (bytes32 encodedId) = abi.decode(performData);
    ...
    function pickWinner(uint256 poolId) private{
