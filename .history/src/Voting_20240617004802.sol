...
contract Voting is AutomationCompatibleInterface, Ownable {
    ...
    function verifySigner712(VotingMessage calldata vote_msg,bytes calldata signature,address signer) external view returns(bool){
        address actualSigner = getSignerEIP712(vote_msg,signature);
        if(signer != actualSigner){
            revert Voting_NotSigner();
        }
	    return true;

    }
    function getSignerEIP712(VotingMessage calldata vote_msg,bytes calldata signature) internal view returns(address){
