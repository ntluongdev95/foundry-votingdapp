...
contract Voting is AutomationCompatibleInterface, Ownable {
    ...
    error Voting_InValidContestantCount();
    ...
    function addContestant (uint256 poolId,string memory name,string memory image,string memory) external {
        if(poolId > s_totalPools){
            revert Voting_PollNotFound();
        }
        if(contestants.length == 0 || contestants.length ==1){
            revert Voting_InValidContestantCount();
        }
