...
contract Voting is AutomationCompatibleInterface, Ownable {
    ...
    function createPool(string memory _name,string memory _title,string memory description, uint256 _startAt, uint256 _endsAt) external {
        if (_startAt <= block.timestamp || _endsAt <= _startAt) {
            revert Voting_InvalidTimeRange();
        }
        if (bytes(_name).length == 0 ||bytes(_title).length == 0 ||bytes(description).length == 0){
