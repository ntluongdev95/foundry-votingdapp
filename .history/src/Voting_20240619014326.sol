...
contract Voting is AutomationCompatibleInterface, Ownable {
    ...
    event WinnerAnnounced(uint256 indexed poolId, uint256 indexed contestantId);
    ///////////////////
    // Modifiers
    ///////////////////
    ...
    function deletePool(uint256 poolId) external {
        ...
        if (msg.sender != owner() && msg.sender != pool.creator) {
            revert Voting_NotOwner();
        }
    ...
    function vote(VotingMessage calldata vote_msg, bytes calldata signature, address signer) external {
        ...
        if(pool.startAt >= block.timestamp || pool.endsAt <= block.timestamp){
            revert Voting_InvalidTimeRange();
        }
    ...
    function getVotingMessageHash(VotingMessage memory vote_msg) public pure returns (bytes32) {
        return keccak256(
            abi.encode(
    ...
    function checkTime(uint256 startingTime, uint256 endingTime) internal view {
    ...
    function updateVoteToContestant(uint256 poolId, uint256 contestantId) private {
         Contestant[] storage contestantsList = s_contestantToPool[poolId];
        for (uint256 i = 0; i < contestantsList.length; i++) {
            if (contestantsList[i].contestantId == contestantId) {
                contestantsList[i].votesCount += 1;
           }
        }
    }

    function getContestants(uint256 poolId) public view returns (Contestant[] memory) {
        return s_contestantToPool[poolId];
    }
    ...
    {    for (uint i = 1; i <= s_totalPools; i++) {
     ...
     function performUpkeep(bytes calldata performData) external override  {
        ...
        for (uint256 i = 0; i < contestantsList.length; i++) {
            if (contestantsList[i].votesCount > highestVotes) {
                highestVotes = contestantsList[i].votesCount;
                winnerIndex = i;
            }
        }

        s_winner[poolId] = contestantsList[winnerIndex];
        emit WinnerAnnounced(poolId, winnerIndex);
